...
#
# Modified 06/06/2023 by Yuxuan Zhuang to work directly with prm+rtf files
# Probably only work with Networkx >= 2.4
#
...
class PRM_FILE(object):
    """PRM file class
    PRM file contains the bonded paramters of the molecule

    Attributes:
        file_name: name of the prm file
        prm_lines: list of lines in the prm file
    """
    def __init__(self, file_name):
        self.file_name = file_name
        self.get_charmm_prm_lines()

    def get_charmm_prm_lines(self):
        foundmol=0
        store=0
        self.prmlines=[]
        section="NONE"
...
class RTF_FILE(object):
    """RTF file class
    RTF file contains the topology of the molecule
    
    Attributes:
        file_name: name of the rtf file
        rtf_lines: list of lines in the rtf file
    """
    def __init__(self, file_name, molname):
        self.file_name = file_name
        self.molname = molname
        self.get_charmm_rtf_lines()

    def get_charmm_rtf_lines(self):
        foundmol = 0
        store = 0
        self.rtplines = []
        section = "NONE"
        with open(self.file_name, "r") as f:
            for line in f.readlines():
                ...
                if line.startswith("RESI"):
                    ...
                    rtfmolname = entry[1]
                    if rtfmolname.casefold() == self.molname.casefold():
                        store = 1
                    else:
                        print("ERROR: Molecule name in RTF file does not match the one in the PRM file")

                if line.startswith("END"):
                    store = 0

                if store == 1:
                    self.rtplines.append(line)
...
class atomgroup:
    ...
    # -----------------------------------------------------------------------
    def write_pdb(self, initpdb_name):
        f = open(initpdb_name, "w")
        for atomi in range(0, self.natoms):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert a mol2 file to a PDB file with LP sites added."
    )
    parser.add_argument("--resname", help="name of residue", default="LIG")
    parser.add_argument("--mol2", help="name of mol2 file", required=True)

    parser.add_argument("--ff", help="name of force field file", required=True)

    # only str or rtf with prm can be specified, not both
    parser.add_argument(
        "--str", help="use str file for atom names"
    )
    parser.add_argument(
        "--rtf", help="use rtf/prm files for atom names"
    )
    parser.add_argument(
            "--prm", help="use rtf/prm files for atom names"
        )
    if parser.parse_args().str and parser.parse_args().prm:
        print("Cannot specify both str and rtf/prm files")
        exit()
...
#    check_versions(rtf_name, ffdir + "/forcefield.doc")
    print("")
    print(
    ...
    m = atomgroup()
    rtf_file = RTF_FILE(rtf_name, molname=mol_name)
    rtplines = rtf_file.rtplines
    print(rtplines)
    ...
    prm_file = PRM_FILE(prm_name)
    prmlines = prm_file.prmlines
    ...
    print(
        f"Additional parameters needed by the molecule are written to {prmfile}, which needs to be included in the system .top"
    )
    print(
